...
class Purchases_model extends CI_Model
{
    ...
    private $table_view = 'purchases';
    private $column_purchase = array(null, 'name', 'email', 'contact', 'total', 'status', 'updated_at', null);
    private $column_search = array('name', 'email', 'contact', 'total', 'status', 'updated_at');
    private $purchase = array('updated_at' => 'desc');
    ...
    {
        $this->db->select('t.*');
        $this->db->select('ot.name as purchase_type');
        ...
        $this->db->join('purchase_types ot', 'ot.id=t.purchase_type_id');
        ...
        if (isset($_POST['purchase'])):
            $this->db->order_by($this->column_purchase[$_POST['purchase']['0']['column']], $_POST['purchase']['0']['dir']);
        elseif (isset($this->purchase)):
            $purchase = $this->purchase;
            $this->db->order_by(key($purchase), $purchase[key($purchase)]);
        endif;
    }
    ...
    public function getProducts($id)
    {
        ...
        $this->db->from('purchase_products t');
        $this->db->join('products p', 'p.id=t.product_id');
        $this->db->where('t.purchase_id', $id);
        $query = $this->db->get();
        return $query->result_array();
    }
