...
class Vendor_login extends REST_Controller
{
    ...
    public function index_post()
    {
        ...
        $object = $this->vendors_model->login();

        $result = [];
        if ($object):
            $this->data['message'] = sprintf($this->lang->line('success_login'), $this->lang->line('text_vendor'));
            $result = $this->getData($object);
        else:
            $this->data['status'] = false;
            $this->data['error'] = sprintf($this->lang->line('error_login'), $this->lang->line('text_vendor'));
        endif;
    ...
    {
        if (!$this->vendors_model->checkUsername($field_value) && $field_value):
